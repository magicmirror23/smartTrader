            ...
            <div *ngFor="let intent of intents" class="intent-row">
              <div class="flex justify-between items-center">
                <div>
                  ...
                  <span class="text-muted" style="margin-left: 8px;">Ã— {{ intent.quantity }}</span>
              ...
              <div class="text-sm text-muted mt-1">
                {{ intent.order_type | uppercase }} Â· ID: <span class="text-mono">{{ intent.intent_id | slice:0:8 }}...</span>
                Â· {{ intent.created_at | date:'short' }}
              </div>
            </div>
          </div>
  ...
  executeIntent(intent: TradeIntent): void {
    ...
    this.tradeApi.execute(intent.intent_id).subscribe({
      next: exec => {
        ...
        this.notify.success(`Trade executed: ${exec.ticker} ${exec.side} Ã— ${exec.quantity} @ ₹${exec.filled_price.toFixed(2)}`);
      },
